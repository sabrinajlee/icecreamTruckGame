...
public class DefeatScreen extends JFrame implements ActionListener {
	...
	private JLabel truckPhoto = new JLabel(new ImageIcon("files/icecreamtruckfire.png"));
	...
	public DefeatScreen() {
		try {
			...
			System.out.println("Error: IOException, error code 8.1");
			e.printStackTrace();

		} catch (Exception e) {
			System.out.println("Error: Unknown exception, error code 8.2");
			e.printStackTrace();
		}
	}
	...
	private void frameSetup() throws IOException {
		...
		setDefaultCloseOperation(DefeatScreen.EXIT_ON_CLOSE);
		setVisible(true);
	}
	...
	private void playDefeatNoise() {
		try {
			...
			AudioInputStream audio = AudioSystem
					.getAudioInputStream(new File("files/wompwomp.wav").getAbsoluteFile());
